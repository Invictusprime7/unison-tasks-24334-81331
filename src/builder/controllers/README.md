...
| 7 | AI-response adapter + opt-in (`src/builder/patch/aiResponseAdapter.ts`) | **done** | `aiResponseToPatchPlan(response, ctx)` converts an `ai-code-assistant` files-map response into a validated `PatchPlan` (create vs. replace inferred from `ctx.existingFiles`, symbol extraction, risk inference, deterministic prompt hash fallback). `isTransactionalOptInEnabled()` reads `localStorage['lovable:patch:transactionalOptIn']` then `VITE_PATCH_TRANSACTIONAL_OPTIN`; defaults to `false`. 12 tests green. AIBuilderPanel flips the flag in a one-line follow-up. |
