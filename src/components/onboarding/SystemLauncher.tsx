...
function buildWizardAiSeedPrompt(opts: {
  ...
  return [
    ...
    opts.designIntervention && opts.designIntervention.experienceBudget !== 'none'
      ? `EXPERIENCE CONTRACT (${opts.designIntervention.experienceBudget}): Immersive 3D is available ONLY through the snapshot-owned "@/unison/experience" modules — never import three, @react-three/fiber, @react-three/drei, or create a raw WebGL canvas. Approved recipes for this site: ${opts.designIntervention.experienceRecipes.join(', ')}. Budget: at most 1 heavy scene per page band, 2 per page. Every scene must degrade to the DOM fallback it already ships with; never rely on WebGL for readable content or intent-bearing CTAs.`
      : '',
