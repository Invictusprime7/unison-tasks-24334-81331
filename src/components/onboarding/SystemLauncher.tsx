...
function buildWizardAiSeedPrompt(opts: {
  ...
  return [
    ...
    ``,
    `LAYOUT CONTRACT (NON-NEGOTIABLE — violations produce scattered/deformed UI):`,
    `- Each section is a top-level <section> with vertical rhythm (py-16 md:py-24) and NO absolute/fixed positioning unless it is a hero background overlay or navbar.`,
    `- Every section's children MUST be wrapped in a centered container: <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">. Never leave headings, paragraphs, or cards as bare section children.`,
    `- Multi-item groups (services, features, team, testimonials, gallery, pricing, logos) MUST use a responsive grid: grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8. Use 4 cols only for >=8 items.`,
    `- Hero, CTA, and content rows MUST use flex with explicit gap utilities (flex flex-col lg:flex-row items-center gap-8 lg:gap-12). No floating siblings, no negative margins to fake layout.`,
    `- Navbar: sticky top-0 z-50, flex items-center justify-between, container wrapper, logo left + nav middle/right + CTA far right.`,
    `- Footer: grid grid-cols-2 md:grid-cols-4 gap-8 inside container, social icon row in flex items-center gap-4.`,
    `- Images: always w-full h-full object-cover inside a fixed aspect-ratio wrapper (aspect-video, aspect-square, aspect-[4/3]). Never let an <img> size itself.`,
    `- Cards: rounded-2xl border bg-card p-6 md:p-8 with consistent internal vertical spacing (space-y-4). All cards in a grid share identical structure.`,
    `- Spacing scale: only Tailwind 4/6/8/12/16/24. Never use arbitrary px values for layout gaps.`,
    `- Z-index: only 0/10/20/40/50. Reserve 50 for navbar/dialog, 40 for sticky CTA.`,
    `- Mobile-first: every grid/flex MUST declare its mobile column count before lg/md breakpoints. Never assume desktop-only viewports.`,
    ``,
    `REACT QUALITY CONTRACT (lint-clean output required):`,
    `- Every .tsx that uses hooks starts with: import React, { useState, useEffect } from "react"; (only hooks actually used).`,
    `- No unused imports, no unused variables, no unused props. Strip them before emitting.`,
    `- All className strings are single concatenated strings (use template literals or cn()). Never spread arrays or call .join inside JSX.`,
    `- All map() callbacks include a stable key (use item.id || index). No missing-key warnings.`,
    `- All <a href="#section-id"> match an existing <section id="section-id"> in the same file. No dead anchors.`,
    `- All event handlers use arrow functions or memoized callbacks; never inline async without await/catch.`,
    `- Every component file ends with a single top-level "export default ComponentName;" — never nested, never duplicated.`,
    `- Balance every brace, paren, bracket, and JSX tag. Self-close void elements (img, br, hr, input). Mentally count tags before finalizing each section.`,
  ].filter(Boolean).join('\n');
}
