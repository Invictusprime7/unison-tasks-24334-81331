/**
 * Intent Registry — single source of truth for every intent the platform
 * recognizes. Every guard in the system (CoreIntent enum, TemplateIntentButton,
 * deterministicIntentUi, aiBindingTool, publish gate) reads from this file.
 *
 * Each entry is derived from the launcher's actual generation surface:
 *  - nav.* — all systems
 *  - commerce.* — store
 *  - booking.* — booking
 *  - auth.* — saas / any system with auth
 *  - lead.* — agency / portfolio / any contact-driven flow
 *  - content.* — newsletter / contact (universal)
 *
