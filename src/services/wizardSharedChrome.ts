...
/* ── Page-body chrome backfill ─────────────────────────────────────────────
 * Chrome authority is the page body, but Lane B sometimes returns a page with
 * no navigation and no footer. Rather than shipping an unreachable page (or
 * failing the launch), the merge wraps that page in this deterministic chrome,
 * derived from the PageRegistry and rendered with canonical design tokens only.
 */

export const PAGE_CHROME_PATH = '/src/components/PageChrome.tsx';

export function buildPageChromeModule(
  registry?: PageRegistry | null,
  businessName = '',
): string {
  const items = buildNavItems(registry);
...
const NAV_ITEMS = ${serializedItems};
const BRAND = ${serializedBrand};
...
export function PageChromeHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur">
      <nav
        className="mx-auto flex w-full max-w-7xl items-center justify-between gap-6 px-5 sm:px-8"
        style={{ minHeight: 'var(--ut-nav-block)' }}
        aria-label="Primary navigation"
      >
        <NavLink to="/" end className="font-heading text-lg font-semibold tracking-tight text-foreground no-underline">
          {BRAND}
        </NavLink>
...
export function PageChromeFooter() {
  return (
    <footer className="border-t border-border bg-muted/40 text-muted-foreground">
      <div className="ut-section mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 sm:px-8">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <p className="font-heading text-base font-semibold text-foreground">{BRAND}</p>
          <nav className="flex flex-wrap gap-x-6 gap-y-2" aria-label="Footer navigation">
            {NAV_ITEMS.map((item) => (
              <NavLink
                ...
                className="font-body text-sm no-underline transition-colors hover:text-foreground"
                data-ut-intent="nav.goto"
                data-ut-path={item.path}
                data-ut-target-page-id={item.pageId}
              >
