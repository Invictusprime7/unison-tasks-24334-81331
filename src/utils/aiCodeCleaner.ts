...
/**
 * Fix SVG markup rendered as raw text strings in React.
 *
 * The AI sometimes generates SVG icons as template-literal strings and then
 * renders them as JSX children — e.g. `<div>{iconStr}</div>` — which causes
 * React to escape the markup and display the raw SVG source as visible text.
 *
 * Handles two patterns:
 *   1. Inline template-literal SVG in a JSX expression child:
 *        {`<svg ...>...</svg>`}
 *      →  <span dangerouslySetInnerHTML={{ __html: `<svg ...>...</svg>` }} />
 *
 *   2. Variable declared as an SVG template-literal string, later used as
 *      a JSX child:
 *        const iconHtml = `<svg ...>...</svg>`;
 *        return <div>{iconHtml}</div>;
 *      →  return <div><span dangerouslySetInnerHTML={{ __html: iconHtml }} /></div>;
 *
 * Also handles plain-string (single/double quoted) SVG assignments for the
 * variable pattern, since AI occasionally uses those instead of backticks.
 */
export function fixSvgStringChildren(code: string): string {
  if (!code || typeof code !== 'string') return code;
  if (!code.includes('<svg')) return code; // fast exit — nothing to do

  let fixed = code;

  // ── Pattern 1: {`...svg...`} inline template-literal JSX child ───────────
  // Matches {`...<svg...></svg>...`} and converts to dangerouslySetInnerHTML.
  // The content regex avoids crossing into a different template literal by
  // using a character class that allows escaped backticks `\`` but not bare ones.
  fixed = fixed.replace(
    /\{(`(?:[^`\\]|\\.)*<svg[\s\S]*?<\/svg>(?:[^`\\]|\\.)*`)\}/g,
    (_m, tpl) => `<span dangerouslySetInnerHTML={{ __html: ${tpl} }} />`,
  );

  // ── Pattern 2: Variable assigned a template/string SVG literal ───────────
  // Collect names of variables whose declared value is an SVG string.
  const svgVarNames: string[] = [];

  // Template-literal form: const/let/var name = `...<svg...`;
  const tplDeclRe = /(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*`(?:[^`\\]|\\.)*<svg[\s\S]*?<\/svg>(?:[^`\\]|\\.)*`\s*;/g;
  let m: RegExpExecArray | null;
  while ((m = tplDeclRe.exec(fixed)) !== null) {
    svgVarNames.push(m[1]);
  }

  // Plain-string form: const/let/var name = "...<svg..." or '...<svg...';
  const strDeclRe = /(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:"[^"]*<svg[\s\S]*?<\/svg>[^"]*"|'[^']*<svg[\s\S]*?<\/svg>[^']*')\s*;/g;
  while ((m = strDeclRe.exec(fixed)) !== null) {
    svgVarNames.push(m[1]);
  }

  // For each detected SVG-string variable, replace bare `{varName}` JSX children
  // with a dangerouslySetInnerHTML span. We only replace patterns that look like
  // JSX children (surrounded by `>`, `<`, newline, or whitespace on both sides)
  // to avoid touching attribute values like `className={varName}`.
  for (const name of svgVarNames) {
    // Escape any regex-special chars in the variable name (should be none but safe)
